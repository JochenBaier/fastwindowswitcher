...
{
  class UIControlViewWalker
  {
  public:
    explicit UIControlViewWalker(IUIAutomation& g_pAutomation);
    ~UIControlViewWalker();

    UIElementPtr UIControlViewWalker::GetFirstChildElement(IUIAutomationElement& p_parent_element) const;
    UIElementPtr GetNextSiblingElement(IUIAutomationElement&  p_element) const;

  private:
    IUIAutomationTreeWalker* m_iui_controller_walker;
  };
};